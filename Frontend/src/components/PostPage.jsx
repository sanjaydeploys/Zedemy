  ...
  const LazySubtitleSection = memo(({ subtitle, index, category, handleImageError }) => {
  ...
  const LazyReferencesSection = memo(({ post, handleImageError }) => {
    const [isVisible, setIsVisible] = useState(false);
    const ref = useRef();
  ...
  const PostPage = memo(() => {
    ...
    const parsedTitle = useMemo(
      () => parseLinks(post?.title || '', post?.category || ''),
      [post?.title, post?.category]
    );
    const parsedContent = useMemo(
      () => parseLinks(post?.content || '', post?.category || ''),
      [post?.content, post?.category]
    );
    const parsedSummary = useMemo(
      () => parseLinks(post?.summary || '', post?.category || ''),
  ...
  { '@type': 'WebPage', '@id': canonicalUrl },
    ...
    // Optional: Enable web-vitals for performance monitoring
    /*k
    useEffect(() => {
      import('web-vitals').then(({ onCLS, onFCP, onLCP, onTBT }) => {
        onCLS(console.log);
        onFCP(console.log);
        onLCP(console.log);
        onTBT(console.log);
