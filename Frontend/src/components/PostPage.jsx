...
const criticalCSS = `
  ...
  h1 { font-size: clamp(1.5rem, 4vw, 2rem); color: #111827; font-weight: 800; margin: 0.75rem 0 0.5rem; line-height: 1.3; }
  section { font-size: 1rem; line-height: 1.6; margin-bottom: 1rem; content-visibility: auto; contain-intrinsic-size: 1px 300px; }
  ...
  p { font-size: 0.875rem; margin-bottom: 0.5rem; }
  @media (min-width: 769px) {
...
const PostContentCritical = memo(({ post, parsedTitle, calculateReadTimeAndWordCount }) => {
  ...
  useEffect(() => {
    ...
    // Extract first ~100 words as plain text for initial render
    const text = post.content.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    const words = text.split(/\s+/);
    const initialWords = words.slice(0, 100).join(' ');
    setInitialContent(initialWords);
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        const wordsInPara = paraText.split(/\s+/).filter(w => w).length;
      ...
      setFullContent({
        aboveFold: aboveFoldContent.join(''),
        belowFold: belowFoldContent.length > 0 ? belowFoldContent.join('') : null,
      });
    }, 2000); // Delay full content rendering to ensure LCP is not blocked
  }, [post?.content]);
  ...
  return (
    <>
      ...
      <ContentSection ref={contentRef}>
        {fullContent ? (
          <>
            <div dangerouslySetInnerHTML={{ __html: fullContent.aboveFold }} />
            {fullContent.belowFold && (
              <div dangerouslySetInnerHTML={{ __html: fullContent.belowFold }} />
            )}
          </>
        ) : (
          <p>{initialContent}</p>
