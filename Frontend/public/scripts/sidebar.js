(function() {
  function initSidebar(maxRetries = 10, retryDelay = 100) {
    let retries = 0;
    ...
    function attemptInit() {
      ...
      if (!sidebarWrapper || !toggleButton) {
        if (retries < maxRetries) {
          retries++;
          setTimeout(() => requestAnimationFrame(attemptInit), retryDelay);
        } else {
          console.error('[sidebar.js] Failed to find sidebar-wrapper or toggle-button after retries');
        }
        return;
      }
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => requestAnimationFrame(attemptInit));
    } else {
      requestAnimationFrame(attemptInit);
    }

    // Persistent DOM observation
    const observer = new MutationObserver((mutations, obs) => {
      if (document.getElementById('sidebar-wrapper') && document.getElementById('toggle-button')) {
        requestAnimationFrame(attemptInit);
        // Keep observing for CSR changes
      }
    });
